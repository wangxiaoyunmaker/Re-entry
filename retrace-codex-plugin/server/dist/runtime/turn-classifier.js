import { TurnClassificationSchema } from "../schemas/stall.js";
...
function inferSameIssue(prompt, activeIssueSummary) {
    if (!activeIssueSummary)
        return { sameIssue: true, confidence: "MEDIUM" };
    const currentTerms = meaningfulTerms(prompt);
    const activeTerms = meaningfulTerms(activeIssueSummary);
    if (currentTerms.length === 0 || GENERIC_RE.test(prompt.trim()))
        return { sameIssue: true, confidence: "HIGH" };
    if (currentTerms.some((term) => activeTerms.includes(term)))
        return { sameIssue: true, confidence: "HIGH" };

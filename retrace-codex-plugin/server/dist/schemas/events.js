...
function contentTextFor(raw) {
    const payload = raw.payload;
    if (raw.hook_event_name === "UserPromptSubmit")
        return asText(payload.prompt);
    if (raw.hook_event_name === "Stop")
        return asText(payload.last_assistant_message);
    if (raw.hook_event_name === "PreToolUse")
        return asText(payload.tool_input?.command);
...
export function normalizeRawHookEvent(rawInput, participantId, collectorSeq) {

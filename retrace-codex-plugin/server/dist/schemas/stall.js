...
function hasInfoGain(informationGain) {
...
export function evaluateStall(input) {
    ...
    const chain = sameChain && existing
        ? {
            ...existing,
    ...
    const reasons = [];
    if (current.intent === "ACCEPTANCE")
        chain.status = "CLOSED";
    if (current.intent !== "CORRECTIVE")
        reasons.push("CURRENT_TURN_NOT_CORRECTIVE");
    if (!current.reportsTargetUnmet)
        reasons.push("TARGET_NOT_REPORTED_UNMET");
    if (!sameChain)
        reasons.push("NEW_OR_DIFFERENT_ISSUE");
    if (chain.unmetReportCount < 2)
        reasons.push("INSUFFICIENT_UNMET_REPORTS");
    if (hasInfoGain(current.informationGain))
        reasons.push("INFORMATION_GAIN_PRESENT");
    if (input.activeReentry)
        reasons.push("ACTIVE_REENTRY");
    if (current.confidence === "LOW")
        reasons.push("LOW_CONFIDENCE");
    if (input.userTurnIndex <= chain.cooldownUntilUserTurn)

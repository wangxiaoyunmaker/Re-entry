...
import { Repository, type PromptAssessmentResult } from "../db/repository.js";
import { RawHookEventSchema, type RawHookEvent } from "../schemas/events.js";
import { classifyPrompt } from "./turn-classifier.js";

export type SubmitAssessment = PromptAssessmentResult & {
  mode: "soft_gate" | "observe_only";
};

export function shouldSoftGate(result: Pick<PromptAssessmentResult, "stallAssessment" | "classification">): boolean {
  return result.stallAssessment.eligible && result.classification.confidence === "HIGH";
}

function rawEventFromHookInput(input: Record<string, unknown>): RawHookEvent {
...
if (process.argv[1] && process.argv[1].endsWith("assess-submit.js")) {
  try {
    ...
    if (result.shouldBlock) process.stdout.write(JSON.stringify({ decision: "block", reason: "ReTrace invitation pending." }));
  } catch (error) {

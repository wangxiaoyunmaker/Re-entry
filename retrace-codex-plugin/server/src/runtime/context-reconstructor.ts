import { AgentClaimSchema, ReconstructionSchema, type AgentClaim, type EvidenceItem, type Reconstruction } from "../schemas/context.js";
import type { IssueChain } from "../schemas/stall.js";
...
function claimKind(text: string): AgentClaim["kind"] {
  if (/(原因|因为|导致|caused by|due to)/iu.test(text)) return "CAUSE";
  if (/(下一步|接下来|next step|建议)/iu.test(text)) return "NEXT_STEP";
  if (/(成功|正常工作|works|working|通过|pass(?:ed)?)/iu.test(text)) return "SUCCESS";
  if (/(完成|修复|解决|fixed|completed|done|已改)/iu.test(text)) return "COMPLETION";
  if (/(现在|当前|状态|目前|is now|currently)/iu.test(text)) return "STATE";
  return "OTHER";
}

function evidenceKind(text: string): EvidenceItem["kind"] {
  if (/(test|测试|spec|vitest|jest|通过|失败)/iu.test(text)) return "TEST_RESULT";
  if (/(build|构建|编译|typecheck|lint)/iu.test(text)) return "BUILD_RESULT";
  if (/(error|错误|异常|失败|exception|stack)/iu.test(text)) return "ERROR";
  if (/(file|文件|代码|source|源码|inspection|检查)/iu.test(text)) return "FILE_INSPECTION";
  return "TOOL_RESULT";
}

export function buildDeterministicReconstruction(input: ReconstructionInput): Reconstruction {
  ...
  const goal = [{
    ...
    sourceType: "CODEX_HOOK" as const,
  ...
  const evidenceItems: EvidenceItem[] = input.events
  ...
  for (const event of input.events.filter((candidate) => candidate.eventType === "AGENT_FINAL" && candidate.contentText)) {
    for (const statement of splitStatements(event.contentText ?? "")) {
      if (!/(完成|修复|解决|成功|原因|因为|导致|下一步|建议|现在|当前|状态|fixed|completed|done|success|caused by|due to|next step|currently|works?)/iu.test(statement)) continue;
  ...
  const explanations = agentClaims.map((claim) => ({
    ...
    text: claim.text,
    kind: "AGENT_HYPOTHESIS" as const,

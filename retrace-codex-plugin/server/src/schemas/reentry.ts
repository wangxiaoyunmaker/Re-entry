...
export type ReentryState = z.infer<typeof ReentryStateSchema>;
...
export type ReentrySnapshot = z.infer<typeof ReentrySnapshotSchema>;

export const PreSurveyResponseSchema = z.object({
  questionSetVersion: z.string().min(1),
  responses: z.record(z.string().min(1), z.number().int().min(1).max(7)).refine(
    (responses) => Object.keys(responses).length > 0,
    "at least one PRE response is required",
  ),
});
export type PreSurveyResponse = z.infer<typeof PreSurveyResponseSchema>;
...
export type PublicReentryContext = z.infer<typeof PublicReentryContextSchema>;
